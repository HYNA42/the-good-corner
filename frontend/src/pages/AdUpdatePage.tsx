...
const AdUpdatePage = () => {
  ...
  // const { getAllCategory, getAlltags } = data;
  ...
  const onSubmit: SubmitHandler<Inputs> = async (formData) => {
    ...
    const dataForBackend = {
      ...
      picturesUrls: formData.picturesUrls.map((pic) => pic.url),
          ...
          <div>
            {fields.map((field, index) => (
              <div key={field.id}>
                <input
                  className="text-field"
                  {...register(`picturesUrls.${index}.url`, {
                    required: "L'URL de l'image est requise",
                  })}
                />
                {errors.picturesUrls?.[index]?.url && (
                  <p className="error error-message">
                    {errors.picturesUrls[index].url?.message}
                  </p>
                )}
